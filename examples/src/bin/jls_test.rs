...
fn client_one_rtt(mut config: ClientConfig) {
...
fn client_zero_rtt(mut config: ClientConfig) {
    // Allow using SSLKEYLOGFILE.
    config.key_log = Arc::new(rustls::KeyLogFile::new());
    //config.jls_config = JlsConfig::new("123", "123");
...
fn server_upstream(mut config: ServerConfig) {
    //config.jls_config = JlsServerConfig::new("123", "123", "localhost::5443");

    let listener = TcpListener::bind(format!("127.0.0.1:{}", 4443)).unwrap();
...
fn test_main() {
    ...
    let server_up = thread::spawn(|| server_upstream(pki.server_config()));

    thread::sleep(time::Duration::from_millis(100));
    client_one_rtt(client_config.clone());
    let client_jls = thread::spawn(move || {

        client_zero_rtt(client_config.clone());
        //client_zero_rtt(client_config);
